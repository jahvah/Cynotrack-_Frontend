...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $account_id = $_SESSION['account_id']; // Session account ID

    // 1️⃣ Get actual recipient_id from recipients_users
    ...
    if (!$recipient) {
        $_SESSION['error'] = "Your recipient profile is missing.";
    ...
    $donor_id     = intval($_POST['donor_id'] ?? 0); // donor used to select specimen
    ...
    // 2️⃣ Check for pending requests by this recipient
    ...
    // 3️⃣ Fetch a stored specimen from this donor
    ...
    // 5️⃣ Insert request into recipient-only table
